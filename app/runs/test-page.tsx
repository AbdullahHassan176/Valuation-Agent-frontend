"use client"

import { useState } from "react"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"

export default function TestRunsPage() {
  const [showChat, setShowChat] = useState(true)

  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <section className="bg-gray-800 border-b border-gray-700 px-8 py-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-white mb-2">Valuation Runs</h1>
            <p className="text-gray-300">Monitor and manage all valuation runs with detailed filtering and analysis</p>
          </div>
          
          <div className="flex items-center space-x-4">
            <div className="text-right">
              <div className="text-sm text-gray-300">Total Runs</div>
              <div className="text-lg font-medium text-white">270</div>
            </div>
            
            <div className="flex space-x-2">
              <Button className="bg-green-600 hover:bg-green-700 text-black font-medium">
                New IRS Run
              </Button>
              <Button className="bg-blue-600 hover:bg-blue-700 text-white font-medium">
                New CCS Run
              </Button>
              <Button className="bg-gray-700 hover:bg-gray-600 text-white font-medium">
                Export List
              </Button>
            </div>
          </div>
        </div>
      </section>

      <section className="px-8 py-6">
        <Card className="bg-gray-800 border-gray-700">
          <CardHeader>
            <CardTitle className="text-lg font-semibold text-white">Test Card</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-gray-300">This is a test card to check if the basic components work.</p>
          </CardContent>
        </Card>
      </section>
    </div>
  )
}
